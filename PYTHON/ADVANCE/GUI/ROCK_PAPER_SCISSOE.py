...
def myGame(user) :
    global user_ch_tk,computer_ch_tk,USER_SCORE,COM_SCORE,u_score,com_score,USER_NAME,display
    ...
    if user==computer_ch :
        ...
        if user=="ROCK"and computer_ch=="PAPER" or user=="PAPER" and computer_ch=="SCISSOR" or user=="SCISSOR" and computer_ch=="ROCK":
            ...
            display_msg.set("COMPUTER IS WON")
            ...
            display_msg.set(f"{USER_NAME.get()} is wOn".upper())
...
btn1 = Button(sc,text="ROCK",width=6,height=1,font=("arial",18,"bold"),bg = "#F59E0B",command=lambda : myGame("ROCK"))
...
#U_SCORE
Label(sc,textvariable=USER_SCORE,font=("arial",10,"bold"),bg="#FF6B35",fg="black").place(x=630,y=200)
...
#display message 
Label(sc,textvariable=display_msg,font=("arial",13,"bold"),width=20 ,bg="white" , fg= "black").place(x=350,y=400)
