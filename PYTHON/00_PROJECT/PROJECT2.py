...
import random
Team_List=['MI','CSK','RR','RCB','GT','LSG','KKR','DD','SRH','PK']
BAT_BALL=['BAT','BALL']
Toss_List=['HEAD','TAIL']
...
while status:
    ...
    U_Toss_choice=input("                                                  Enter Your Toss Choice HEAD/TAIL  :".upper()).upper()
    if U_Toss_choice ==Toss_List[0][0] or U_Toss_choice==Toss_List[1][0] or U_Toss_choice==Toss_List[0] or U_Toss_choice==Toss_List[1]:
        if U_Toss_choice==Toss_Result[0] or U_Toss_choice==Toss_Result:
            ...
            if User_Turn==BAT_BALL[0]:
                    ...
                    BAT_FIRST.append(User_Team)
            else:
                print(f"\n\n        *=*=*=*=*=*=*={OP_Team} IS BATTING   *=*=*=*=*=*=*=")
                BAT_FIRST.append(OP_Team)
        else:
            OP_Turn=random.choice(BAT_BALL)
            print(f"OPPSITE WIN TOSE AND CHOSE {OP_Turn}")
            if OP_Team==BAT_BALL[0]:
                print(f"\n\n        *=*=*=*=*=*=*={OP_Team} IS BATTING   *=*=*=*=*=*=*=")
                BAT_FIRST.append(OP_Team)
            else:
                print(f"\n\n        *=*=*=*=*=*=*={User_Team} IS BATTING   *=*=*=*=*=*=*=")
        ...
        print(f"Re-Enter  :".upper())
    
    if BAT_FIRST[0]==User_Team:      
        print("--------------------Let's Play-------------------")
        status=True
        free_hit=True
        while status:
            ...
            if USER_SCORE_BORD[3]<20 and USER_SCORE_BORD[1]<10:
                ...
                print(f"\n\n\n\n{OP_Team} NEEED {USER_SCORE_BORD[0]+1} TO WIN")
                status=True
                while status:
                        ...
                        if OP_SCORE_BOARD[0]<USER_SCORE_BORD[0]:
                            ...
                            if input:
                                ...
                                if BALL_ACT==BALL_PROB[7]:
                                    ...
                                    OP_SCORE_BOARD[2]+=1
                                    print(f"SCORE BOARD :{User_Team} :{OP_SCORE_BOARD[0]}/{OP_SCORE_BOARD[1]} {OP_SCORE_BOARD[3]}.{OP_SCORE_BOARD[2]} (overs)")
                                print(f"{OP_Team} NEED {USER_SCORE_BORD[0]+1-OP_SCORE_BOARD[0]} RUN IN {19-OP_SCORE_BOARD[3]}.{6-OP_SCORE_BOARD[2]}")
                        else: 
                            print(f"{OP_Team} WON THE MATCH BY {2-OP_SCORE_BOARD[1]} WICKET")
                            status=False
                    else:
                        print(f"{User_Team} WON MATCH BY {USER_SCORE_BORD[0]-OP_SCORE_BOARD[0]}")
                        status=False
    else:
        status=True
        while status:
            ...
            if OP_SCORE_BOARD[3]<1 and OP_SCORE_BOARD[1]<2:
                ...
                if input:
                    ...
                    print(f"{User_Team} NEEED {OP_SCORE_BOARD[0]+1} TO WIN")
                    while status:
                        if USER_SCORE_BORD[2]==6:
                            USER_SCORE_BORD[2]=0
                            USER_SCORE_BORD[3]+=1
                        if USER_SCORE_BORD[3]<1 and USER_SCORE_BORD[1]<2:
                            if USER_SCORE_BORD[0]<OP_SCORE_BOARD[0]:
                                print(f"{User_Team} NEED {OP_SCORE_BOARD[0]-USER_SCORE_BORD[0]} RUN IN {6-USER_SCORE_BORD[2]}")
                                BALL_PROB=[0,1,2,3,4,5,6,"WICKET","WIDE","NO BALL"]
                                input("PressEnter>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                                if input:
                                    BALL_ACT=random.choice(BALL_PROB)
                                ...
                                print(f"{User_Team} WON THE MATCH BY {2-USER_SCORE_BORD[1]} WICKET")
                                status=False    
                        else:
                            print(f"{OP_Team} WON THE MATCH {OP_SCORE_BOARD[0]+1-USER_SCORE_BORD[0]} RUN") 
                            status=False